...
>
    ...
    <div class="flex items-center justify-end gap-1 mt-1.5">
        {{-- Deadline --}}
        <div @if ($card->sprint->status === 'active') x-data="{ editing: false }" @endif wire:key="deadline-{{ $card->id }}">
            <div 
                @if ($card->sprint->status === 'active') @click="editing = true" @endif
                class="flex items-center gap-2 bg-gray-200 dark:bg-gray-900 rounded-md px-2.5 py-1.5 {{ ($card->sprint->status === 'active') ? 'cursor-pointer' : 'cursor-help' }}"
                ...
                <ul wire:key="users-{{ $card->id }}-{{ $card->assignees->count() }}" class="h-48 p-2 text-sm overflow-y-auto" id="user-list-{{ $card->id }}">
                    @foreach ($filteredUsers as $user)
                        ...
                        <li class="user-item flex items-center p-2 hover:bg-gray-300 rounded"
                            ...
                            <label class="w-full flex items-center justify-between {{ ($card->sprint->status === 'active') ? 'cursor-pointer' : '' }}">
