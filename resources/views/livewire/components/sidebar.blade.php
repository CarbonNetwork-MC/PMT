<div id="sidebar-navbar" class="hidden lg:block">
    <div class="z-40 w-full h-screen transition-transform -translate-x-full sm:translate-x-0">
        <div class="h-full flex flex-col px-3 py-4 overflow-y-auto bg-carbon-900 dark:bg-gray-800">
            ...
            <ul class="flex flex-col flex-grow justify-between space-y-2 font-medium">
                <div>
                    ...
                    @if ($user->role->id !== 1)
                        ...
                        @if ($user->role && json_decode($user->role->permissions)->view_other_projects)
                            <li class="side-item-container">
                                <x-sidebar-nav-link href="{{ route('admin.manage-projects.render') }}" :active="request()->routeIs('admin.manage-projects.render')">
                        ...
                        @if ($user->role && json_decode($user->role->permissions)->manage_users)
                            <li class="side-item-container">
                                <x-sidebar-nav-link href="{{ route('admin.manage-users.render') }}" :active="request()->routeIs('admin.manage-users.render')">
                ...
                <div class="flex justify-between text-white">
                    <div class="font-rw-black">
                        {{ config('app.name') }} - {{ config('app.version') }}
                    </div>
                    <div class="font-rw-regular hover:text-sky-500 hover:font-bold">
                        <a href="{{ route('utils.bug-report.render') }}" class="">Report a bug</a>
                    </div>
                </div>
            </ul>
        </div>
